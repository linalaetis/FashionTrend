...
SECRET_REGEXES = {
    ...
    "Generic API Key/Token": re.compile(
        r"(?i)(api[_-]?key|apikey|x-api-key|secret|token|access[_-]?token)\s*[:=]\s*['\"][0-9A-Za-z_\-]{16,}['\"]"
    ),
...
DEFAULT_ALLOWLIST_REGEXES = [
    ...
    r"\b[0-9a-fA-F]{32}\b",  # MD5 / UUID compact
...
def any_allowlisted(s: str, allowlist_res: list[re.Pattern], ignore_hashes: bool) -> bool:
    for rgx in allowlist_res:
        if rgx.search(s):
            if not ignore_hashes and rgx.pattern in (
                r"\b[0-9a-fA-F]{32}\b",
                r"\b[0-9a-fA-F]{40}\b",
                r"\b[0-9a-fA-F]{64}\b",
            ):
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Scan de secrets (fichiers, notebooks, historique Git) + résumé du rapport."
    )
    ap.add_argument(
        "--history", action="store_true", help="Scanner aussi l'historique Git (plus lent)."
    )
    ...
    ap.add_argument(
        "--skip-outputs",
        action="store_true",
        help="Ignorer les sorties des notebooks (réduit le bruit).",
    )
    ap.add_argument(
        "--min-entropy", type=float, default=4.5, help="Seuil d'entropie pour alerter (défaut=4.5)."
    )
    ap.add_argument(
        "--allow-pattern",
        action="append",
        default=[],
        help="Regex à ignorer (peut être passée plusieurs fois).",
    )
    ap.add_argument(
        "--exclude",
        action="append",
        default=[],
        help="Dossiers supplémentaires à exclure (peut être passée plusieurs fois).",
    )
    ap.add_argument(
        "--no-ignore-hash",
        action="store_true",
        help="Ne pas ignorer les chaînes hex de 32/40/64 chars.",
    )
    ap.add_argument(
        "--summary-only",
        action="store_true",
        help="Ne pas scanner: seulement résumer le fichier --out existant.",
    )
    args = ap.parse_args()
    ...
    for p in paths:
        if p.suffix == ".ipynb":
            ...
            try:
                ...
                continue
            scan_text(
                text, str(p), findings, allowlist_res, args.min_entropy, not args.no_ignore_hash
            )
